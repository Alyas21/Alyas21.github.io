...
// g++ Wire_World.cpp -o Wire_World.exe -lgdi32
// ./Wire_World.exe
...
int check_elec(int x, int y) {
    int lneigh = 0;
    if (noutbounds(x - 1, y + 1) && plain[y + 1][x - 1] == 2) {
        lneigh++;
    }
    if (noutbounds(x, y + 1) && plain[y + 1][x] == 2) {
        lneigh++;
    }
    if (noutbounds(x + 1, y + 1) && plain[y + 1][x + 1] == 2) {
        lneigh++;
    }
    if (noutbounds(x - 1, y) && plain[y][x - 1] == 2) {
        lneigh++;
    }
    if (noutbounds(x + 1, y) && plain[y][x + 1] == 2) {
        lneigh++;
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        case WM_PAINT: {
            ...
            for(int y = 0; y < height; y++) {
                for(int x = 0; x < width; x++){
                    int nElecHead = check_elec(x, y);
                    if ((plain[y][x] == 1 && (nElecHead == 1 || nElecHead == 2)) || plain[y][x] == 2) {
                        change_cells.push_back(make_pair(2, make_pair(x, y)));
                    }
                    if(plain[y][x] == 2) {
                        change_cells.push_back(make_pair(3, make_pair(x, y)));
                    }
                    if (plain[y][x] == 3) {
